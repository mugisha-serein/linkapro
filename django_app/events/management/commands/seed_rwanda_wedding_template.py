...
def budget_rows(workbook):
    ...
    header_index = next(
        (index for index, row in enumerate(rows) if any("item" in str(cell).lower() or "cost item" in str(cell).lower() for cell in row if cell)),
        None,
    )
    if header_index is None:
        return []
    headers = [str(value or "").lower() for value in rows[header_index]]
    item_index = next((i for i, value in enumerate(headers) if "item" in value), 0)
    category_index = next((i for i, value in enumerate(headers) if "category" in value), item_index)
    estimate_indexes = [i for i, value in enumerate(headers) if "estimated cost" in value or "low (rwf)" in value]
    estimate_index = estimate_indexes[-1] if estimate_indexes else next(
        (i for i, value in enumerate(headers) if "cost" in value and "actual" not in value and "unit" not in value),
        min(item_index + 1, len(headers) - 1),
    )
    notes_index = next((i for i, value in enumerate(headers) if "note" in value), None)
    parsed = []
    for row in rows[header_index + 1 :]:
        ...
        if not item or "total" in str(item).lower() or str(item).strip().endswith("."):
            continue
        estimate = decimal_value(row[estimate_index] if estimate_index < len(row) else 0)
        if not isinstance(row[estimate_index] if estimate_index < len(row) else None, (int, float, Decimal)):
            continue
        ...
        parsed.append((str(category or "Other"), str(item), estimate, str(notes or "")))
    return parsed
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        docs_dir = Path(settings.BASE_DIR) / "wedding_docs"
        if not docs_dir.exists():
            docs_dir = Path(__file__).resolve().parents[4] / "wedding_docs"
        if not docs_dir.exists():
            raise CommandError(f"Wedding workbook directory not found: {docs_dir}")
        ...
        for stage_order, (name, slug, filename) in enumerate(STAGES, start=1):
            ...
            if slug == "church-wedding" and len(workbook.worksheets) > 1:
                questions.extend(
                    str(row[0])
                    for row in workbook.worksheets[1].iter_rows(min_row=2, values_only=True)
                    if row[0]
                )
        ...
        self.stdout.write(self.style.SUCCESS(f"Seeded {template.slug}: {summary}"))
