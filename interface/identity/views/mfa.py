...
from interface.identity.shared.oauth_state_wiring import (
...
from interface.identity.services import get_command_handlers, get_google_login_use_case, get_google_oauth_adapter
from interface.identity.shared.cookies import (
    clear_auth_cookies,
...
from interface.identity.shared.serializers import TwoFactorLoginSerializer, TwoFactorSetupVerifySerializer
from interface.identity.throttles import (
    AuthRateLimited,
    GoogleOAuthIPThrottle,
    TwoFactorEnrollmentThrottle,
    TwoFactorIPThrottle,
...
class EnableTwoFactorView(APIView):
    ...
    throttle_classes = [TwoFactorEnrollmentThrottle]
...
class GoogleLoginView(APIView):
    permission_classes = [AllowAny]
    throttle_classes = [GoogleOAuthIPThrottle]

    def throttled(self, request, wait):
        raise AuthRateLimited(wait=wait, request=request)

    def get(self, request):
        from interface.identity.shared.oauth_state_wiring import ALLOWED_OAUTH_SIGNUP_ROLES
