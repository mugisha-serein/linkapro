...
from interface.identity.shared.cookies import extract_refresh_token
from interface.identity.throttles import get_client_ip, rate_limit_hash
...
class SessionManagementRateLimited(APIException):
    status_code = 429
    default_code = "session_management_rate_limited"

    def __init__(self, wait=None, request=None):
        self.wait = math.ceil(wait) if wait else None
        super().__init__("Too many session management attempts. Please try again later.")
        self.detail = api_error_payload(
            code=self.default_code,
            message="Too many session management attempts. Please try again later.",
            request=request,
        )
...
class SessionManagementIPThrottle(TokenEndpointThrottle):
    scope = "session_management_ip"
    env_rate_name = "SESSION_MANAGEMENT_IP_RATE"
    default_rate = "30/min"
    rate_limited_event = "session_management_rate_limited"
...
class SessionManagementUserThrottle(TokenEndpointThrottle):
    scope = "session_management_user"
    env_rate_name = "SESSION_MANAGEMENT_USER_RATE"
    default_rate = "60/hour"
    rate_limited_event = "session_management_rate_limited"
    ...
    def get_fingerprint(self, request) -> str | None:
        ...
        if not user_id:
            return None
        return rate_limit_hash(str(user_id))
