...
class EmailVerificationRateLimited(APIException):
    status_code = 429
    default_code = "email_verification_rate_limited"

    def __init__(self, wait=None, request=None):
        self.wait = math.ceil(wait) if wait else None
        super().__init__("Too many email verification attempts. Please try again later.")
        self.detail = api_error_payload(
            code=self.default_code,
...
class TwoFactorEnrollmentThrottle(AuthEndpointThrottle):
    scope = "two_factor_enrollment"

    def get_cache_key(self, request, view):
        account_id = getattr(getattr(request, "user", None), "id", None)
        if not account_id:
            return None
        view_name = view.__class__.__name__
        fingerprint = f"{view_name}:{account_id}:{get_client_ip(request)}"
        return self._cache_key(rate_limit_hash(fingerprint))
...
class EmailVerificationEndpointThrottle(AuthEndpointThrottle):
    def _log_limited(self, request, *, reason: str) -> None:
        logger.warning(
...
class EmailVerificationIPThrottle(EmailVerificationEndpointThrottle):
...
class EmailVerificationEmailThrottle(EmailVerificationEndpointThrottle):
    scope = "email_verification_email"

    def get_cache_key(self, request, view):
        email = getattr(getattr(request, "user", None), "email", "")
        return self._cache_key(rate_limit_hash(str(email).strip().lower()))
...
def clear_login_failures(request, email: str | None = None, *, user_id=None) -> None:
    # The login use case records failures keyed by the raw (normalized) email,
    # so clearing must use that same key. Passing a pre-hashed fingerprint here
    # would hit a different cache key (the repository hashes the account key
    # again), silently leaving the failure counter in place and keeping the
    # account locked after a successful login.
    account_key = _normalize_email_value(email) or _normalized_email(request)
    email_hash = rate_limit_hash(account_key)
    try:
        _account_lockout_service().record_success(account_key)
    except Exception as exc:
        logger.error(
