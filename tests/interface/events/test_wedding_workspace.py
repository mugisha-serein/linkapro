...
from interface.events.template_seeders.rwanda_wedding import STAGES, ensure_rwanda_wedding_template
from interface.events.workspace_service import generate_event_workspace
from interface.vendors.models import ServicePackage
...
def test_generate_event_workspace_raises_for_rwanda_wedding_missing_template(monkeypatch):
    ...
    monkeypatch.setattr("interface.events.workspace_service.ensure_rwanda_wedding_template", missing_template)
...
def test_wedding_event_creation_rolls_back_when_workspace_clone_fails(planner_client, monkeypatch):
    ...
    monkeypatch.setattr("interface.events.views.generate_event_workspace", clone_failure)
    response = client.post(
        reverse("event-list"),
        {
            "name": "Broken Wedding",
