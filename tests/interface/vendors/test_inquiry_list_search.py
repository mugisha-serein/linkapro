...
from interface.vendors.views.inquiries import InquiryListView
...
def test_inquiry_list_view_passes_q_as_search_text():
    ...
    with patch(
        "interface.vendors.views.inquiries._get_current_vendor_profile",
        return_value=(profile, None),
    ), patch(
        "interface.vendors.views.inquiries.get_query_handlers",
