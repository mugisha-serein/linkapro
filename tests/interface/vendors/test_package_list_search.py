...
from interface.vendors.views.packages import ServicePackageListView
...
def test_package_list_view_passes_q_as_search_text():
    ...
    with patch(
        "interface.vendors.views.packages._get_current_vendor_profile",
        return_value=(profile, None),
    ), patch(
        "interface.vendors.views.packages.get_query_handlers",
        return_value=handlers,
    ), patch(
        "interface.vendors.views.packages._augment_package_response",
