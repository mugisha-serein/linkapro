...
def test_production_settings_pass_with_required_email_env():
    result = _import_settings("interface.settings.production", _production_env())
...
def test_development_and_test_settings_allow_local_http_vault_addresses():
    ...
    env.update(
        {
            "PYTHONPATH": str(PROJECT_ROOT),
            "DJANGO_SETTINGS_MODULE": "interface.settings.development",
    ...
    result = _run_settings_snippet(
        "\n".join(
            [
                "from interface.settings import development, test",
...
def test_production_settings_default_token_env_is_production():
    result = _run_settings_snippet(
        "\n".join(
            [
                "from interface.settings import production as settings",
...
def test_local_and_test_settings_do_not_require_sendgrid():
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            "import interface.settings.development; import interface.settings.test; print('ok')",
        ],
        cwd=PROJECT_ROOT,
        env=env,
        text=True,
