...
from interface.identity.models import PasswordResetEmailDelivery, User
from interface.vendors.models import PortfolioImage, ServicePackage, VendorProfile, VerificationDocument
...
class TestVendorApprovalAdmin:
    ...
    def test_admin_email_health_reports_unhealthy_config(self, caplog):
        ...
        api_client = APIClient()
        api_client.force_authenticate(user=admin_user)
        caplog.set_level(logging.ERROR, logger="interface.governance.views")
    ...
    def test_approve_vendor_action(self, admin_client, monkeypatch):
        monkeypatch.setattr(
            "interface.governance.vendor_command_handlers.sync_or_delete_vendor_projection",
    ...
    def test_admin_api_approval_syncs_marketplace_listing(self, admin_client, monkeypatch):
        calls = []
        monkeypatch.setattr(
            "interface.governance.views.sync_vendor_to_marketplace",
    ...
    def test_admin_api_suspend_and_reinstate_vendor_updates_marketplace(self, admin_client, monkeypatch):
        ...
        synced = []
        monkeypatch.setattr(
            "interface.governance.views.delete_vendor_from_marketplace",
