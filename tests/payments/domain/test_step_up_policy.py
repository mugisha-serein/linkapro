...
class TestStepUpPolicy:
    def test_below_threshold_no_step_up(self):
        money = Money(minor_units=100_000, currency=Currency("RWF"))  # 100k RWF
        result = StepUpPolicy.is_step_up_required(money, token_step_up=False, now=utc_now())
        assert result.required is False

    def test_above_threshold_without_step_up_requires(self):
        money = Money(minor_units=600_000, currency=Currency("RWF"))  # 600k RWF
        result = StepUpPolicy.is_step_up_required(money, token_step_up=False, now=utc_now())
        assert result.required is True
        assert "exceeds threshold" in result.reason

    def test_above_threshold_with_step_up_allows(self):
